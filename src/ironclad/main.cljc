(ns ironclad.main
  "Kotodama WASM entrypoint for the Ironclad clean-room actor (L4) — Clojure port.
  ...
  facts (`ironclad.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "ironclad")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"ironclad.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "ironclad-compat" :tier tier :entities entities} 200])
...
(defn start! [] :ironclad-compat/ready)
